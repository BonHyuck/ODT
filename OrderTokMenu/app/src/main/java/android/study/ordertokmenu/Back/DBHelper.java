...
public class DBHelper extends SQLiteOpenHelper {
    ...
    public void insertStoreInfo(StoreInfo storeInfo){
        // 이미 정보가 있는지 확인
        String checkSql = "SELECT id, storeId, storeName, storeRandom, tableNumber, updateTime, posIpAddress, employeeIpAddress, backImgPath, logoImgPath, resetCode FROM storeInfo";
        ...
        String sql = "INSERT INTO storeInfo (storeId, storeName, storeRandom, tableNumber, updateTime, posIpAddress, employeeIpAddress, backImgPath, logoImgPath, resetCode) VALUES (?,?,?,?,?,?,?,?,?,?)";
        SQLiteDatabase db = getWritableDatabase();
        Object[] data = {storeInfo.getStoreId(), storeInfo.getStoreName(), storeInfo.getStoreRandom(), storeInfo.getTableNumber(), storeInfo.getUpdateTime(), storeInfo.getPosIpAddress(), storeInfo.getEmployeeIpAddress(), storeInfo.getBackImgPath(), storeInfo.getLogoImgPath(), storeInfo.getResetCode()};
        db.execSQL(sql, data);
    }
    // 테이블 번호 바꾸기
    public void changeTableNumber(int tableNumber){
        String sql = "UPDATE storeInfo SET tableNumber = ?";
        SQLiteDatabase db = getWritableDatabase();
    ...
    public StoreInfo getStoreInfo(){
        ...
        String sql = "SELECT storeId, storeName, storeRandom, tableNumber, updateTime, posIpAddress, employeeIpAddress, backImgPath, logoImgPath, resetCode FROM storeInfo";
        ...
        if(cursor != null) {
            if (cursor.getCount() > 0) {
                ...
                int indexResetCode = cursor.getColumnIndex("resetCode");
                ...
                storeInfo.setResetCode(cursor.getString(indexResetCode));
            }else{
                storeInfo = null;
            }
        }else{
